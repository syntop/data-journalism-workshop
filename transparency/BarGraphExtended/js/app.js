...
dsv("data/Facebook_Global_Government_Requests_2013_Jan-June.csv", function(error, data) {
	...
	data.sort(function(a,b) { return +b.Total_Requests < +a.Total_Requests ? -1 : +b.Total_Requests > +a.Total_Requests ? 1 : 0; });

	var min = d3.min(data, function(d) { return +d.Total_Requests;} );
	var max = d3.max(data, function(d) { return +d.Total_Requests;} );
	var valuescale = d3.scale.log().domain([min,max]).range([0,barwidth]);
    ...
    bar.append("svg:rect")
  	.attr("class", "bar_facebook_percentage")
	.attr("x", baroffsetx)
    .attr("y", function(d, i){return i * (barheight + gap) + baroffsety})
    .attr("width", function(d, i){return (valuescale(d.Total_Requests) * d.Percentage / 100);})
    .attr("height", barheight)
    ;
    ...
    bar.append("svg:text")
  	.attr("class", "value")
	.attr("transform", function(d, i) {
            var x = valuescale(d.Total_Requests) + baroffsetx;
            var y = i * (barheight + gap) + baroffsety + barheight;
            return "translate("+x+","+y+")";
          })
    .text(function(d){return d.Total_Requests;})
    ;
});
